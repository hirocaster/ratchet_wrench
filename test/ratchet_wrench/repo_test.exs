defmodule RatchetWrench.RepoTest do
  ...
  test "insert/get/delete new record at auto uuid value" do
    ...
    # UUIDv4
    assert byte_size(struct.singer_id) == 36
    ...
    d = DateTime.utc_now()
    # ex) "13:23"
    utc_now_string = String.slice("#{d}", 11, 5)
  ...
  test "get where record, use secondary index" do
    ...
    {:ok, singers_list} =
      RatchetWrench.Repo.where(%Singer{}, where_sql, params, "singers_by_first_name")
  ...
  test "convert value type for TIMESTAMP" do
    ...
    converted_date_time =
      RatchetWrench.Repo.convert_value_type("#{from_cloudspanner_dt}", "TIMESTAMP")

    {:ok, converted_date_time} =
      DateTime.shift_zone(converted_date_time, "Etc/UTC", Tzdata.TimeZoneDatabase)
  ...
  describe "return SQL" do
    ...
    test ".get_sql" do
      ...
      assert get_sql ==
               "SELECT * FROM user_items WHERE user_id = @user_id AND user_item_id = @user_item_id"
    end
    ...
    test ".insert_sql/1" do
      ...
      assert insert_sql ==
               "INSERT INTO singers(first_name, inserted_at, last_name, singer_id, updated_at) VALUES(@first_name, @inserted_at, @last_name, @singer_id, @updated_at)"
    end
    ...
    test ".delete_sql/1" do
      ...
      assert delete_sql ==
               "DELETE FROM user_items WHERE user_id = @user_id AND user_item_id = @user_item_id"
    end
    ...
    test ".update_sql/1" do
      ...
      assert update_sql ==
               "UPDATE singers SET first_name = @first_name, inserted_at = @inserted_at, last_name = @last_name, updated_at = @updated_at WHERE singer_id = @singer_id"
    end
  end
  ...
  test ".paramTypes/1" do
    assert RatchetWrench.Repo.param_types(Singer) == %{
             first_name: %{code: "STRING"},
  ...
  describe "type cast checks" do
    ...
    test "get test data by .where/3" do
      {:ok, test_data_list} =
        RatchetWrench.Repo.where(%Data{}, "data_id = @data_id", %{data_id: "test_data"})
    ...
    test "Update record in INTERLEAVE child table" do
      ...
      update_child_log =
        Map.merge(child_log, %{message: "update child table record in interleave"})
